...
class InputHelper(object):
    ...
    def _process_targets(arguments):
        def pre_process_target_spec(target_spec, split_comma=True):
            target_spec = "".join(
            ...
            if split_comma:
                ...
                # Don't split by comma - treat entire line as one target
                return [target_spec] if target_spec else []

        if arguments.target:
            # -t flag: support comma-separated targets inline
            target_specs = pre_process_target_spec(arguments.target, split_comma=True)
        else:
    ...
    def process_data_for_tasks_iterator(arguments):
        ...
        tasks = list()
        quiet = getattr(arguments, 'quiet', False)  # Quiet mode suppresses command output
...
class InputParser(object):
    ...
    def setup_parser():
        ...
        # Allow version check without requiring target
